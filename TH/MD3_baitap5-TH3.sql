...
-- Khi truyền tham số dạng IN
delimiter &&
drop procedure if exists getngaysinh &&
create procedure getngaysinh(in sinh date)
begin
select * from dmsv where NgaySinh=sinh;
end &&
delimiter &&
call getngaysinh('1990-12-20');

-- Khi truyền tham số dạng OUT
 delimiter &&
 drop procedure if exists getgioitinh &&
 create procedure getgioitinh(in gt varchar(7),out cotgt varchar(50))
 begin 
 select makhoa into cotgt from dmsv where tensv=gt;
 end &&
 delimiter &&
 
 call getgioitinh('mai',@vietanh);
 select @vietanh as viet_anh ;
drop procedure pro_getCusbyid